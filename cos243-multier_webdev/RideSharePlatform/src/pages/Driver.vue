    ...
    <v-container>
      ...
      <v-row v-if="driverID != -1">    <!-- Welcome message and large buttons if logged in -->
        ...
        <v-col cols="2" align-self="center">
          <!-- <v-btn class="primary" v-on:click="newRide">New Ride</v-btn>   -->
          <v-dialog v-model="dialog">   <!-- new ride popup dialogue box  -->
            <template v-slot:activator="{ on }">
              <v-btn color="red lighten-2" v-on="on">New Ride
              </v-btn>
            </template>
            <v-card>
              <v-card-title class="headline grey lighten-2" primary-title>Create a ride</v-card-title>
              <v-card-text>
                This is where you'd add all those paramiters! Text boxes, dorp downs, everything to create a new ride.
              </v-card-text>
              <v-divider></v-divider>
              <v-card-actions>
                <v-spacer></v-spacer>
